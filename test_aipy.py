...
def processTask(folder: str, task: str, activatedNeuronNames: list[str]):
    ...
    mappingNeuron = Neuron("mapping", lambda: dict(), [], dict[int, int])
    associateNeuron = Neuron("associate", utils.associate, [dict[int, int], int, int], dict[int, int])
    mapNeuron = Neuron("map", primitives.map, [list[np.ndarray], dict[int, int]], list[np.ndarray])
    inferColorMappingNeuron = Neuron("inferColorMapping", primitives.inferColorMapping, [list[tuple[np.ndarray, np.ndarray]]], dict[int, int])
